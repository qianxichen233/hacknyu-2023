...
export default async function handler(req, res) {
    ...
    for (let i = 0; i < 6 && i < hackathonData.length; i++) {
        if (typeof hackathonData[i].date != "undefined") {
            ...
            flightSource["name"] =
                flight_dummy_data["legs"][0]["origin"]["name"];
            flightSource["displayCode"] =
                flight_dummy_data["legs"][0]["origin"]["display_code"];

            flightDestination["name"] =
                flight_dummy_data["legs"][0]["destination"]["name"];
            flightDestination["displayCode"] =
                flight_dummy_data["legs"][0]["destination"]["display_code"];
            ...
            flightObject["totalDuration"] =
                flight_dummy_data["totalDuration"] / 120;
            flightObject["Carriers"] =
                flight_dummy_data["legs"][0]["carriers"][0]["name"];
